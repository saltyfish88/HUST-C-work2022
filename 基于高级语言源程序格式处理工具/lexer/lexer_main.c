...
while(nextstep(file,i++,&present,&next)!=0)//等于0则文件结束，跳出循环。
{
	...
	else if(isalpha(present)!=0||present=='_')//判断是否是标识符。 
	{
		...
		{
			k++;
			if(strcmp(keywords[k-1],tokenarray[j-1].value)==0)//若为关键字，则把type设置成相应的关键字。 
			{
	...
	else//判断分隔符和运算符。 
	{
		switch(present)
		{
			...
			default:
				tokenarray[j++].type=ERR;
				lexerr(TOKEN_NO_EXIST);//错误的词法记号
				break;
		}
	}
}
tokenarray[j].type=END;//标记结束 
tokenarray[j].loc=lin;
token_print_to_stdout(tokenarray);//按格式整理到标准输出
token_print_to_file(Token,tokenarray);//输出到文件 
fclose(Token);
return 0; 
}
